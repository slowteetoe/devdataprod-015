...
shinyUI(fluidPage(
  ...
  inputPanel(
    ...
    sliderInput("age", label="Age", min=16, max=120, step=1, value=30),
    selectInput("education", label="Education Level:", choices=sort(unique(as.character(adult$education))), selected=""),
    selectInput("marital.status", label="Marital Status:", choices=sort(unique(as.character(adult$marital.status))), selected=""),
    selectInput("occupation", label="Occupation:", choices=sort(unique(as.character(adult$occupation)))),
    selectInput("race", label="Ethnicity:", choices=sort(unique(as.character(adult$race)))),
    ...
    sliderInput("hours.wk", label="# Hours Worked per Week:", min=1, max=100, step=1, value=40),
